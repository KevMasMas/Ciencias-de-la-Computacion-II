...
class Graph:
    ...
    def has_edge(self, vertex_1: Any, vertex_2: Any):
        if (vertex_2 in self.adjancency_list[vertex_1]) and (vertex_1 in self.adjancency_list[vertex_2]):
            return True
        return False
    
    def get_neighboors(self, vertex: Any) -> Optional[List[Any]]:
        if vertex not in self.vertices:
    ...
    def bfs(self, start: Any) -> List[Any]:
        if start not in self.vertices:
            raise ValueError(f'El vertice {start} no exite.')
        ...
        while not queue.is_empty():
            ...
            for neighbor in self.get_neighboors(vertex):
    ...
    def dfs(self, start: Any) -> List[Any]:
        if start not in self.vertices:
            raise ValueError(f'El vetice {start} no existe')
